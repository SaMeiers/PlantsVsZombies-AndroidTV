...
#include <pvz_tv/elf32/elf32_defs.h>
...
#define PVZ2_DATA_IMPORT_MAX  64u
...
int pvz2_elf_load(const char *path, uint32_t space_size, pvz2_elf_image_t *out) {
    ...
    /* Map the libraries the game dlopen()s at runtime, if they ship next to the
     * main .so: libHomura (the mod) and libGameRegister (exports GetSerialFile,
     * which the license check in CommonAuthManager::DoAuthenticate requires --
     * without it the game shuts itself down once loading finishes). Homura
     * stays first so its base address does not move. */
    static const char *const kRuntimeModules[] = { "libHomura.so", "libGameRegister.so" };
    for (size_t r = 0; r < sizeof(kRuntimeModules) / sizeof(kRuntimeModules[0]); ++r) {
        char mod_path[1024];
        snprintf(mod_path, sizeof(mod_path), "%s%s", dir, kRuntimeModules[r]);
        if (!file_exists(mod_path)) continue;
        if (out->module_count >= PVZ2_MAX_MODULES) {
            fprintf(stderr, "elf32_loader: module table full, cannot map %s\n", kRuntimeModules[r]);
            pvz2_elf_free(out);
            return -1;
        }
        uint32_t base = (next_base + PVZ2_MODULE_ALIGN - 1) & ~(PVZ2_MODULE_ALIGN - 1);
        uint32_t index = out->module_count;
        if (map_module(out, mod_path, base, index, NULL) != 0) {
            fprintf(stderr, "elf32_loader: failed to map '%s'\n", mod_path);
            pvz2_elf_free(out);
            return -1;
        }
        out->module_count = index + 1;
        next_base = base + out->modules[index].span;
        printf("elf32_loader:   mod '%s' mapped at 0x%08x..0x%08x (%u KB, %u exports)\n",
               kRuntimeModules[r], base, base + out->modules[index].span,
               out->modules[index].span >> 10, out->modules[index].dynsym_count);
    }
