...
/* div_t div(int numer, int denom) -- an 8-byte struct, so under AAPCS it comes
 * back through a hidden result pointer in r0 and the arguments shift to r1/r2. */
void c_div(GuestCall &c) {
    std::uint32_t out = c.arg(0);
    std::int32_t numer = (std::int32_t)c.arg(1), denom = (std::int32_t)c.arg(2);
    if (out != 0 && c.in_bounds(out, 8) && denom != 0) {
        c.write32(out, (std::uint32_t)(numer / denom));
        c.write32(out + 4, (std::uint32_t)(numer % denom));
    }
    c.set_result(out);
}
...
void c_qsort(GuestCall &c) {
    ...
    bool is_fast_compar = (compar == gameMainBase + 0x004A9CE0 || compar == gameMainBase + 0x004AAC64);

    std::vector<std::uint32_t> indices(nmemb);
    for (std::uint32_t i = 0; i < nmemb; ++i) indices[i] = i;

    if (is_fast_compar) {
        std::stable_sort(indices.begin(), indices.end(),
                         [&](std::uint32_t a_idx, std::uint32_t b_idx) {
                             uint32_t pa = c.read32(base + a_idx * size);
                             uint32_t pb = c.read32(base + b_idx * size);
                             uint32_t ka = (pa && c.in_bounds(pa, 4)) ? c.read32(pa) : 0;
                             uint32_t kb = (pb && c.in_bounds(pb, 4)) ? c.read32(pb) : 0;
                             return ka < kb;
                         });
    } else {
        std::stable_sort(indices.begin(), indices.end(),
                         [&](std::uint32_t a_idx, std::uint32_t b_idx) {
                             const std::uint32_t args[2] = {base + a_idx * size, base + b_idx * size};
                             return (std::int32_t)c.call_guest(compar, args, 2) < 0;
                         });
    }

    std::vector<std::uint8_t> sorted(nmemb * size);
    for (std::uint32_t i = 0; i < nmemb; ++i) {
        std::memcpy(&sorted[i * size], &c.img->mem[base + indices[i] * size], size);
    }
    std::memcpy(&c.img->mem[base], sorted.data(), nmemb * size);
}
...
void c_bsearch(GuestCall &c) {
    ...
    while (lo < hi) {
        ...
        std::int32_t r = 0;
        if (is_fast_compar) {
            uint32_t pa = c.read32(key);
            uint32_t pb = c.read32(base + mid * size);
            uint32_t ka = (pa && c.in_bounds(pa, 4)) ? c.read32(pa) : 0;
            uint32_t kb = (pb && c.in_bounds(pb, 4)) ? c.read32(pb) : 0;
            if (ka < kb) r = -1;
            else if (ka > kb) r = 1;
            ...
            r = (std::int32_t)c.call_guest(compar, args, 2);
        }
