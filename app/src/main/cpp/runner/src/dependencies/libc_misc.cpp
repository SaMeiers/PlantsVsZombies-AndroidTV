...
void c_mmap(GuestCall &c) {
    ...
    uint32_t length = c.arg(1);
    uint32_t prot = c.arg(2);
    uint32_t flags = c.arg(3);
    ...
    uint32_t aligned_len = (length + 0xFFFu) & ~0xFFFu;
    uint32_t alloc = c.rt ? c.rt->heap.alloc(aligned_len) : 0;
    if (alloc == 0) {
        c.log("[-] [libc] mmap() -- ENOMEM (requested %u bytes)", length);
...
/* munmap: free guest memory allocated by mmap */
void c_munmap(GuestCall &c) {
    uint32_t addr = c.arg(0);
    uint32_t length = c.arg(1);
    if (c.rt && addr != 0 && addr != (std::uint32_t)-1) {
        c.rt->heap.free_ptr(addr, c.lr());
    }
    c.set_result(0);
}
